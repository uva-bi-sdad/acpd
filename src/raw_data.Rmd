...
abc_violations <- function() {
  if (!dir.exists(paths = "data/acpd/original/abc_violations")) {
    dir.create(path = "data/acpd/original/abc_violations")
    }
  ...
  periods <- c("2017-07-2018-06.xls",
               ...
               "2015-07-2016-06.xls",
               "2014-07-2015-06.xls")
  destfiles <- here(str_c("data/acpd/original/abc_violations/", periods))
...
abc_licensees <- function() {
  ...
  if (!dbExistsTable(conn = conn, name = "abc_licensees")) {
    read_excel("data/acpd/original/abc_licensees.xlsx") %>%
      dbWriteTable(con = conn,
                   name = 'abc_licensees',
                   value = .,
                   row.names = FALSE)
...
ari_restaurants_layer <- function() {
  destfile <- here("data", "acpd", "original", "ari_restaurants_laye.geojson")
  if (!file.exists(destfile)) {
    download.file(url = "https://opendata.arcgis.com/datasets/2dc91350ea6f4bffb65ece8730b7fcca_0.geojson",
                  destfile = destfile)
    }
  conn <- dbConnect(drv = PostgreSQL(),
                    host = "postgis",
  ...
  if (!dbExistsTable(conn = conn, name = "ari_restaurants")) {
    st_read(dsn = here("data", "acpd", "original", "ari_restaurants_laye.geojson")) %>%
    st_write(dsn = conn)
    }
  on.exit(expr = dbDisconnect(conn = conn))
  }
