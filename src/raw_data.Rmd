...
crime_incidents <- function() {
  ...
  list.files(path = here("data", "acpd", "original")) %>%
    ...
    subset(str_extract(string = ., pattern = "\\d{4}-\\d{2}-\\d{2}") != today_is) %>% file.remove()
      ...
      map_df(.f = function(path) {
        read_excel(path = path,
                   ...
                   col_names = c("record_control_number", "name", "license", "address",
                                 ...
                                 "revoked", "dismissed", "other", "x","x", "x","x", "x","x","x"),

                   skip = 1L)
        }) %>% data.table() %>%
      dt_filter(!is.na(license)) %>%
      dt_arrange(license) %>%
      dbWriteTable(con = conn,
                   name = 'abc_violations',
                   value = .,
