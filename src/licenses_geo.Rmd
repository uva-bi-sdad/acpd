...
geo_key = substr(geo_data_complete$address, 1, 6)
str_sub()
license_key = substr(licenses$Add1, 1, 6) 

#MAKE UNIQUE KEY COL FOR BOTH DATASETS BEFORE MERGE
str_c
geo_data_complete$unique_key <- paste(geo_key,geo_data_complete$restaurant)
licenses$unique_key <- paste(license_key,licenses$TradeName)

licenses <- licenses[c("TradeName","Add1","unique_key","LicStatus_StatusDesc")]
