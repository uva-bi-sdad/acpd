      ...
      tabItem(
        ...
        fluidRow(
          box(width = 16,
              ...
              h3("Research Team"),
              div("DSPG: Alexa Nosal (VT), Kelsey McMahon (VT), Megan Grondine (VT), Hannah Brinkley (VT) , and Bayoán Santiago Calderón (DSPG Fellow, CGU)"),
              div("SDAL: Gizem Korkmaz and Aaron Schroeder"),
              div("Sponsors: Jim Mastoras, Arlington County Police Department; and the Virginia Tech Center for Peace Studies and Violence Prevention (CPSVP)")
          )
        )
      ),
...
server <- shinyServer(function(input, output, session) {
  ...
  observeEvent(input$dd1, {
    output$crimesdata <- DT::renderDT(make_datatable(input$dd1, crimes_data))
  })
