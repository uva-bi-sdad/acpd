...
for (pkg in c("DBI", "RPostgreSQL", "maditr", "geosphere", 'purrr', 'dplyr', 'stringr', 'lubridate')) {
...
refresh_filtered_incidents <- function(){
  ...
  acpd_filtered <- acpd_data %>%
    ...
    dt_mutate(day = weekdays(as_date(acpd_data$start)), 
              hour = as.numeric(hour(acpd_data$start))) %>%
    ...
    dt_mutate(nightlife = (day %in% c("Thursday", "Friday", "Saturday", "Sunday") | str_detect(string = start, pattern = str_c(c("05-05 | 05-06 | 03-17 | 03-18"), collapse = "|"))) & (hour %in% c(1:5,21:24)))  %>% 
      ## Filter for crime-type
      dt_filter(crime_type)

  acpd_filtered$crime_type <- NULL
